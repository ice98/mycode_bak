...
#error "please define __VC6_ATL_IMAGE_H__ flag in stdafx.h file's begin and following..."
    ...
    inline HDC CImage::CDCCache::GetDC() throw()
    {
        ...
        typedef LONG (CALLBACK *LPFNInterlockedExchange)(LPLONG, LONG);
        LPFNInterlockedExchange lpfnInterlockedExchange;

        HMODULE hModule = GetModuleHandle("kernel32.dll");
        if (hModule)
        {
            lpfnInterlockedExchange = (LPFNInterlockedExchange)GetProcAddress(hModule, "InterlockedExchange");

            if (NULL == lpfnInterlockedExchange)
            {
                return NULL;
            }
        }
        ...
        {
            // hDC = static_cast< HDC >((*lpfnInterlockedExchange)( reinterpret_cast<void** >(&m_ahDCs[iDC]), NULL ) );

            hDC = reinterpret_cast< HDC >((*lpfnInterlockedExchange)(reinterpret_cast<LPLONG>(&m_ahDCs[iDC]), NULL) );
    ...
    {
        typedef PVOID (CALLBACK *LPFNInterlockedExchangePointer)(PVOID *Target, PVOID Value);
        LPFNInterlockedExchangePointer lpfnInterlockedExchangePointer;
        
        HMODULE hModule = GetModuleHandle("kernel32.dll");
        if (hModule)
        {
            lpfnInterlockedExchangePointer = (LPFNInterlockedExchangePointer)GetProcAddress(hModule, "InterlockedExchangePointer");
            
            if (NULL == lpfnInterlockedExchangePointer)
            {
                AfxMessageBox("error,CImage::CDCCache::ReleaseDC");
                return;
            }
        }
        ...
        for( int iDC = 0; iDC < CIMAGE_DC_CACHE_SIZE; iDC++ )
        {
            ...
            hOldDC = static_cast< HDC >((*lpfnInterlockedExchangePointer)(reinterpret_cast< void** >(&m_ahDCs[iDC]), hDC ) );
